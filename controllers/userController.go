...
import (
	...
	helper "golang-hotell-app/helpers"
	"golang-hotell-app/models"
	"log"
	"net/http"
	"time"
...
func VerfifyPassword(userPassword string, providedPassword string) (bool, string) {
...
func SignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured when cheking the phone"})

		}
		if count > 0 {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "email or password already exist"})
		}
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		passwordIsValid, msg := VerfifyPassword(*user.Password, *foundUser.Password)
		defer cancel()
